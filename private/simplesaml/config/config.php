 ...
 */
if (!defined('SAML_SALT')) {
    $secrets_json_text = file_get_contents($_ENV['HOME'].'/files/private/secrets.json');
    $no_salt = '.x.x.x.salt';
    $no_pass = 123;
    if (false === $secrets_json_text) {
        define('SAML_SALT', $no_salt);
        define('SAML_ADMIN_PAW', $no_pass);
    } else {
        $secrets_data = json_decode($secrets_json_text, true);
        define('SAML_SALT', isset($secrets_data['SAML_SALT']) ? $secrets_data['SAML_SALT'] : $no_salt);
        define('SAML_ADMIN_PAW', isset($secrets_data['SAML_ADMIN_PAW']) ? $secrets_data['SAML_ADMIN_PAW'] : $no_pass);
    }
}
...
$config = [
    ...
    */
     ...
     */
    'auth.adminpassword' => SAML_ADMIN_PAW,
