            ...
            <ul class="view-type">
              <li>
                <a
                  href=""
                  title="Detailed view"
                  @click.prevent="setView('StaffCard')"
                >
                  <unicon
                    name="document-layout-left"
                    :style="{'fill' : (activeView === 'StaffCard' ? '#037abc' : '#6d7278' )}"
                  />
                </a>
              </li>
              ...
              <li>
                <a
                  ...
                  @click.prevent="setView('StaffSimple')"
                >
                  <unicon
                    name="list-ul"
                    :style="{'fill' : (activeView === 'StaffSimple' ? '#037abc' : '#6d7278' )}"
                  />
                </a>
              </li>
            </ul>
...
export default {
  ...
  data () {
    return {
      activeView: typeof initialView !== 'undefined' && initialView === 'collapsed' ? 'StaffSimple' : 'StaffCard', // eslint-disable-line no-undef
      activeViewIcon: true,
      loading: true,
  ...
  async updated () {
    // Scroll to requested staff member (via URL hash) after loading is complete
    // -- wait for entire page, including images (thanks to forceNextTick)
    // -- had to double it up (wait two ticks) to ensure all images are rendered by browser
    // -- https://github.com/twickstrom/vue-force-next-tick
    var ref = this
    await this.$forceNextTick()
    await this.$forceNextTick()
    if (typeof ref.$refs.staff !== 'undefined') {
