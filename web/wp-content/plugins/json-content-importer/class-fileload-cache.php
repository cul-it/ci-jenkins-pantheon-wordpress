...
class FileLoadWithCache {
  ...
  public function __construct($feedUrl, $urlgettimeout, $cacheEnable, $cacheFile, $cacheExpireTime, $oauth_bearer_access_key, $http_header_default_useragent_flag, $debugmode, $fallback2cache=0){
  ...
  /* get errorlevel */
	...
	private function storeFeedInCache(){
	  ...
	  if (is_null($tmpJSON)) {
        ...
        if (is_null($tmpJSON2)) {
			...
			$this->debugmessage .= "cache-error:<br>feed is not valid JSON - not cached";
			$this->showdebugmessage("cache-error:<br>feed is not valid JSON - not cached");
			return "";
		}
	  }
	...
	public function func_fallback2cache($respHttpCode){
		$this->showdebugmessage("use fallback2cache: ".$this->fallback2cache);
		...
		if ($useCachedJson) {
			if(file_exists($this->cacheFile)) {
				...
				$this->showdebugmessage("fallback2cache ok: cached JSON found");
				$this->allok = TRUE;
			} else {
				$this->showdebugmessage("fallback2cache failed: no cached JSON found");
			}
		}
	}
