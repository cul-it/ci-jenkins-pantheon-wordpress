...
class JsonContentParser123 {
    ...
    /* retrieveDataAndBuildAllHtmlItems: get json-data, build html*/
      ...
      if (!is_object($jsonTree) && !is_array($jsonTree)) {
        $this->debugEcho("<b>We have a Problem with JSON here:</b><br>Either we got no JSON from the API. Or the basenode-parameter is not ok.<br>Switch on the Debugmode of the Plugin!", "wordpressticket");
    ...
    */
    ...
    private function debugEcho($txt, $paramIn="", $object=NULL) {
      ...
      if ($paramIn=="wordpressticket") {
        $this->errormsg .= $txt.'<br><b>Switch to gutenberg-Blocks!</b><br>There the JSON Content Importer Block gives an easier way to use the JSON-APIs.<br>Or: <b>Switch on the Debugmode</b> by adding "debugmode=10" in the Shortcode.<p>If all is without success: Open ticket at <a href="https://wordpress.org/support/plugin/json-content-importer" target="_blank">wordpress.org</a> please<hr>';
      }
