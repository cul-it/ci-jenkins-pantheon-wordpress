...
class Wp_Custom_Post_Loops extends Widget_Base
{
	...
	protected function _register_controls() {
		...
		if (strstr( $domain, 'africana' )) {
			$options = array_merge($options, $africana);
		} elseif (strstr( $domain, 'engineering' )) {
			$options = array_merge($options, $engineering);
		} elseif (strstr( $domain, 'catherwood' )) {
			$options = array_merge($options, $ilr);
		} elseif (strstr( $domain, 'law' )) {
			$options = array_merge($options, $law);
		} elseif (strstr( $domain, 'johnson' )) {
			$options = array_merge($options, $management);
		} elseif (strstr( $domain, 'mathematics' )) {
			$options = array_merge($options, $math);
		} elseif (strstr( $domain, 'physicalsciences' )) {
			$options = array_merge($options, $psl);
		} elseif (strstr( $domain, 'rare' )) {
			$options = array_merge($options, $rare);
		}
