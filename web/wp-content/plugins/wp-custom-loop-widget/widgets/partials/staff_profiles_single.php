...
<section class="staff-profile" aria-label="<?php echo $first_name ?> <?php echo $last_name ?> staff profile">
  <?php if (!empty($image)) : ?>
    <img class="staff-photo" src="<?php echo $image['url']; ?>" alt="">
  <?php else : ?>
  ...
  <h3>
    <?php if (!empty(get_field('first_name')) && !empty(get_field('last_name'))) :
      ...
      if (!empty(get_field('degree'))) : echo ", "; the_field('degree'); endif;
    endif; ?>
  </h3>
  ...
  <h4>
    <?php if (!empty(get_field('title'))) :
      the_field('title');
      if (!empty(get_field('departments'))) :  
        $terms_departments = get_field('departments');
        $all_terms_departments = "";
        if( !empty( $terms_departments ) ): 
          foreach( $terms_departments as $term ): 
            $all_terms_departments = $all_terms_departments . $term->name . ", ";
          endforeach; 
          echo ', ' . substr_replace($all_terms_departments ,"",-2);
        endif; 
      endif;
    endif; ?>
  </h4>
  ...
  <?php 
  $terms_areas_of_expertise = get_field('areas_of_expertise');
  $all_terms_areas_of_expertise = "";
  if( !empty( $terms_areas_of_expertise ) ): 
    foreach( $terms_areas_of_expertise as $term ): 
      $all_terms_areas_of_expertise = $all_terms_areas_of_expertise . $term->name . ", ";
    endforeach; 
    echo '<p class="staff-liaison"><strong>Areas of Expertise: </strong>' . substr_replace($all_terms_areas_of_expertise ,"",-2);
  endif; 
  ?>
  
  <?php 
  $terms_liaison_areas = get_field('liaison_areas');
  $all_terms_liaison_areas = "";
  if( !empty( $terms_liaison_areas ) ): 
    foreach( $terms_liaison_areas as $term ): 
      $all_terms_liaison_areas = $all_terms_liaison_areas . $term->name . ", ";
    endforeach; 
    echo '<p class="staff-liaison"><strong>Liaison Areas: </strong>' . substr_replace($all_terms_liaison_areas ,"",-2);
  endif; 
  ?>
