...
<div class="database_metadata">
  <?php if( !empty(get_field('database_url'))) : ?>
    <div class="button">
      <button href="<?php echo the_field('database_url') ?>">Connect to <?php echo the_field('database_title'); ?> <span class="fa fa-external-link"></span></button>
    </div>
  <?php endif; ?>
  <?php if (!empty(get_field('database_status'))) : ?>
    <div>
      <p class="status">
        <?php $status_arg = get_field('database_status');
        if ($status_arg) : ?>
          <ul>
