...
class Wp_Libcal_Hours_Public {
  ...
  private $libcal_hours_weekly = NULL;
	...
	/*
	* Get the data for todays hours from libcal and cache it for 1 mintue
	* to reduce the number of api calls to libcal
	*/
	private function get_libcal_hours_today($atts) {
	...
	*/
	private function get_location_data($atts) {
		$data = $this->get_libcal_hours_today($atts);
	...
	public function libcal_header_hours_func($atts) {
		...
		$status = $this->libcal_status_now_func($atts);
		$hours_today = $this->libcal_hours_today_func($atts);
		if (get_option('libcal_library_id_2') != NULL) {
			if ($lid == '2818') {
				$unit_name = 'Olin';
			} else if ($lid == '2830') {
				$unit_name = 'Uris';
		...
		$html = '<span id="header-hours-' . $lid . '">';
		//$html .= '<i class="fas fa-clock icon-time" aria-hidden="true"><span class="sr-only">Library hours status</span></i>';
		if ($unit_name != NULL) {
			$html .= '<span class="libcal-unit-name">' . $unit_name . ': </span>';
		}
		$html .= '<span class="libcal-status-now">' . $status . ' </span>';
		$html .= '<span class="libcal-hours-today">' . $hours_today . '</span>';
		//$html .= '- <a class="full-hours" href="/full-hours">Full Hours</a> /';
		$html .= '</span>';
		return $html;
	}
	/*
	...
	*/
	public function libcal_status_now_func($atts) {
  	...
  	if ($location_data->lid == 7862 || $location_data->lid == 10937) {
			if ($location_data->times->text != NULL) {
        $currently_open = true;
        $status = NULL;
      } elseif ($location_data->times->text == NULL && $location_data->rendered != NULL) {
        ...
        $status = NULL;
			...
			$status = ($currently_open ? "Open" : "Closed -");
				...
				if ($currently_open) {
					...
					// Format to render friendly hours
					switch ($hours) {
						case "24hours":
							$hours = "24 hours";
							break;
					...
					if ($hours == '24 hours' || $hours == 'Study space open 24 hours' || $hours == '24/7 Study Space & Computing Only' || $hours == '24/7 Study Space & Computing') {
						$hours_text = $hours;
					} else {
          ...
          if ($location_data->times->text != NULL) {
            ...
            if ($hours == '24 hours' || $hours == 'Study space open 24 hours' || $hours == '24/7 Study Space & Computing Only' || $hours == '24/7 Study Space & Computing') {
              $hours_text = "Closed";
            } else if ($hours == 'Request/pickup service available') {
              $hours_text = ' - ' . $hours;
            }
          } else {
 ...
 // Render custom prev next/week month button to navigate the weekly/monthly calendar
  ...
  private function get_libcal_hours_weekly($atts) {
    $weekly_transient_key = 'libcal_hours_weekly_data';
    $weekly_data = get_transient( $weekly_transient_key );
    if ($weekly_data === false) {
      ...
      $weekly_hours_endpoint = $this->libcal_hours_weekly_endpoint . '&lid=' . $lid . '&weeks=52';
      $weekly_request = wp_remote_get($weekly_hours_endpoint);
      if (is_wp_error($request)) {
        return false;
      }
      $weekly_body = wp_remote_retrieve_body($weekly_request);
      $weekly_data = json_decode($weekly_body);
      $expiration = 60; // 1 minute
      set_transient( $weekly_transient_key, $weekly_data, $expiration );
    }
    return $weekly_data;
  }
  ...
  public function libcal_hours_sidebar_today_func($atts) {
    ...
    $weekly_location_data = $this->get_libcal_hours_weekly($atts);
    $loc_id = 'loc_'.$lid;
