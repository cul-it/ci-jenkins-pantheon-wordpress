...
function drag_drop(event) {
    ...
    drag_drop_upload_method(fileobj);      
}
function drag_drop_upload_method(fileobj){
    var formData = new FormData();
    var filesArray = fileobj;
...
function upload_method(){
    ...
    jQuery.ajax({
        ...
        target:   '#targetLayer',
        success: function (uploaded_file_info) {           
            if(uploaded_file_info == 'Invalid XML'){
                swal({
                                title: 'Invalid XML',
                                text: "XML Parsing Error: not well-formed.",
                                type: "warning",
            ...
            document.getElementById('wp_notice').innerHTML = '<p>Your csv file has invalid UTF-8 character. please check your csv</p>';
                    ...
                    if(check_file != "csv" && check_file != "zip" && check_file != "xml" && check_file != "txt" || file.error == 'Filetype not allowed' || file.isvalid == 'No') {
                        warning = 'Un Supported File Format';
            if(file.isvalid == 'No'){
                    //swal('Warning!', 'Your csv file columns and values are mismatch', 'error')
                warning = 'Your csv file columns and values are mismatch';
            }
                        swal({
                                title: warning,
                                text: "You will not be able to upload this file!",
                    ...
                    if(check_file != 'zip') {
                        jQuery.ajax({
                            ...
                            success: function (priority_result) {
                                ...
                                if(priority_result != '' && priority_result['is_template'] != 'yes' && check_file == 'xml'){
                                    action += '&mapping_type=advanced';
                                }
                                jQuery('#form_import_file').attr('action', action);
                                fndisplayprogressbar();
                                jQuery('.continue-btn').attr('disabled', false);
...
jQuery( function() {
    jQuery( '#dwn_ftp_file' ).click(function() {
        jQuery.ajax({
            ...
            if(uploaded_file_info['isutf8'] == 'No'){
                        document.getElementById('wp_notice').style.display = '';
                        document.getElementById('wp_notice').innerHTML = '<p>Your csv file has invalid UTF-8 character. please check your csv</p>';
                    }
                    if(fileextn == 'zip') {
...
function external_method() {
    ...
    if (validateUrl(external_url)==true) {
        
        jQuery.ajax({
        type: "POST",
        url:ajaxurl,
        data: {
            'action': 'upload_from_url_check',
            'url_tocheck': external_url,
        },
...
function igniteImport() {
   ...
   /* window.onbeforeunload = function(){
    ...
    var rollback_mode = document.getElementById('rollback_mode').value;
    ...
    jQuery.ajax({
        ...
        success: function (response) {
            ...
            console.log(total);
            if(currentlimit == 2 && msg2 != null){
              jQuery('#innerlog').append(jQuery("<p style='margin-left:10px;color:green;'>"+msg2+"</p>"));
            }
        ...
        if(rollback_mode == 'on')
                document.getElementById('rollback_now').style.display = "";
                return false;
            } else {
...
function rollbacknow() {
    ...
    swal({
        title: 'Are you sure?',
        text: "Rollback will restore your wordpress site prior to this import job, please confirm as this action is irreversable.",
        type: "warning",
        showCancelButton: true,
        confirmButtonColor: "#DD6B55",
                ...
                if(data == 'Rollback Completed') {
                    swal({
                       ...
                       // text: data,
                        ...
                        function(){
                            jQuery('#rollback_now').hide();
                            jQuery('#img_ajax').hide();
                            var currentURL = location.protocol + '//' + location.host + location.pathname + '?page=sm-uci-import';
                            window.location = currentURL;
                            return true;
                        });
...
function delete_schedule(id, type) {
...
jQuery(function(){
    ...
    jQuery('#schedule_export').on('ifChecked', function(){
        jQuery('#scheduled_export').show();
        jQuery('#proceed_to_export').fadeOut('slow', function(){
            jQuery('#scheduled_export_btn').fadeIn();
        });
    });

    jQuery('#schedule_export').on('ifUnchecked', function(){
        jQuery('#scheduled_export').hide();
        jQuery('#scheduled_export_btn').hide();
        jQuery('#proceed_to_export').show();
    });
});
...
function igniteExport() {
    //var cb = jQuery('#split_chkbox').val();
    var is_check_split = 'no';
    if(jQuery('#split_chkbox').prop( "checked" )) {
       var is_check_split = 'yes';
    }
    // alert('cb');
    // return false;
    var typeExp = jQuery('#ex_ty').val();
    ...
    if(is_data_with_specific_exclusions == true && Object.keys(exclusion_header_list).length == 0){
        var warning="Please choose any field to continue Export";
        swal("Warning!", warning, "warning");
        return false;
    }
    jQuery.ajax({
        ...
        success: function (response) {
            if(response != null) {
             ...
             //jQuery("a#download_file_link").attr("href", 'admin.php?page=sm-uci-export&exportType=export-with-filters&export_file='+response.exported_file+'&export_type='+response.export_type);
                jQuery('#offset').val(response.new_offset);
                if ((parseInt(response.total_row_count) + parseInt(response.limit)) < parseInt(response.new_offset)) {
...
function static_method(id,prefix, mappingcount, staticheader){
    ...
    if(mappingvalue == 'header_manip'){
        jQuery.ajax({
            ...
            success: function (data) {
                var val = jQuery('#'+group+'_statictext_mapping'+mappingcount).val();
                document.getElementById(group+'_customdispdiv_mapping'+mappingcount).innerHTML = data;
                jQuery('#'+group+'_statictext_mapping'+mappingcount).val(val);
...
//alert(id+name);
   ...
   //s3 
     if(id =='enable_s3'){
    if(document.getElementById('enable_s3').checked) {
     jQuery('#awss3starts').removeClass('awss3starts');
     jQuery('#awss3starts').addClass('awss3enable');
    }else{
     jQuery('#awss3starts').removeClass('awss3enable');
     jQuery('#awss3starts').addClass('awss3starts');
    }
     }
   //s3
//alert(name+value);
...
function retrieve_record(action, value) {
    ...
    var xmlid = jQuery('#xml_tag_name').val();
    var treetype = jQuery('#tree_type').val();
    if(action == 'prev') {
    ...
    if(is_xml == 1){
        ...
        success: function (response) {
            console.log('success');
            jQuery('ul.uci_mapping_attr_value').html(response);
...
function scheduleExport() {
    var datetoschedule = jQuery('#datetoschedule').val();
    var host_name = jQuery('#host_name').val();
    var host_port = jQuery('#host_port').val();
    var host_username = jQuery('#host_username').val();
    var host_password = jQuery('#host_password').val();
    var host_path = jQuery('#host_path').val();
    var schedule_frequency = jQuery('#schedule_frequency').val();
    var schedule_time = jQuery('#timetoschedule').val();
    var scheduled_info = {
        'date': datetoschedule,
        'host_name': host_name,
        'host_port': host_port,
    ...
    if(datetoschedule == '') {
        swal('Warning!', "Don't leave Scheduled Date as empty!", 'warning')
    } else if(host_name == '') {
        swal('Warning!', "Don't leave Hostname as empty!", 'warning')
    } else if(host_port == '') {
        swal('Warning!', "Don't leave Host Port as empty!", 'warning')
    } else if(host_username == '') {
        swal('Warning!', "Don't leave Host Username as empty!", 'warning')
    } else if(host_password == '') {
        swal('Warning!', "Don't leave Host Password as empty!", 'warning')
    } else if(host_path == '') {
        swal('Warning!', "Don't leave Host Path as empty!", 'warning')
    }

    // Save all data to schedule export
    var exclusion_header_list = JSON.parse( "" || "{}");
    var filterOptions = new Array('getdatawithdelimiter', 'getdataforspecificperiod', 'getdatawithspecificstatus', 'getdatabyspecificauthors', 'getdatabasedonexclusions');
    var items = jQuery("form :input").map(function(index, elm) {
        return {id: elm.id, name: elm.name, type:elm.type, value: jQuery(elm).val()};
    ...
    jQuery.ajax({
        ...
        dataType: "json",
        data: {
            'action': 'parseDataToScheduleExport',
            'module': module,
            'optionalType': optionalType,
            'conditions': conditions,
            'eventExclusions': eventExclusions,
        ...
        success: function (response) {
            console.log(response);
            var currentURL = location.protocol + '//' + location.host + location.pathname + '?page=sm-uci-export';
            swal({
                    title: 'Success!',
                    text: response.msg,
                    type: "success",
...
function getXmlElement(id, pag = 1, nextpag = null) {

    var path = document.getElementById('xml_file_path').value;

    document.getElementById('pag_tab').style.display = 'block';
      if(nextpag != null){
        var str = id;
        id = str.replace("pag", "");
      }else{
        document.getElementsByClassName('sam')[0].value = 1;
        document.getElementsByClassName('sam')[0].id = id+'pag';
        jQuery('.xml-aside div').removeClass('active');
        jQuery('#'+id).addClass('active');
      }
      jQuery('.xml-loader').addClass('active');
    document.getElementById('tagname').value = id;
      ...
      jQuery.ajax({
        ...
        success: function (response) {
            console.log('success');
            document.getElementById('res_data').innerHTML = response;
            jQuery('.xml-loader').removeClass('active');
        },
        error: function (errorThrown) {
            console.log(errorThrown);
        }
...
function changeVal(argument) {

    var val = document.getElementsByClassName('sam')[0];
    var str = val.id;
    id = str.replace("pag", "");
    var check = document.getElementById(id+'span').innerHTML;
    ...
    getXmlElement(val.id, val.value, 'yes');

}

function expColXml(id) {
    jQuery('#icon_'+id).toggleClass('glyphicon-plus-sign glyphicon-minus-sign');
   jQuery('#div'+id).toggle();
}

function goToAdvMode(argument) {
    var tgname = document.getElementById('tagname').value;
    var url = window.location.href;
    if(tgname==""){
        return false;
    }
    else{
     window.location.replace(url + '&tag_name='+tgname);
    }
    
}

function changeTreeView(type) {
    //alert(type);
    var url = window.location.href;
    var getUrlParameter = function getUrlParameter(sParam) {
        var sPageURL = decodeURIComponent(window.location.search.substring(1)),
    ...
    var get_type = getUrlParameter('tree_type');
    if(type == 'table') {
            window.location.replace(url + '&tree_type=table');
    } else {
            window.location.replace(url + '&tree_type=tree');
            //jQuery.param.querystring(window.location.href, 'mapping_type=advanced');
    }
}
...
//aws work
function aws_data(){
    var s3region = jQuery('#aws-region').val();
    if(s3region == '--Select--'){
    swal('Warning!', 'Please select a region', 'warning');  
    return false;
    }
    var key = document.getElementById('aws-key').value;
    var secretkey = document.getElementById('aws-secret-key').value;
     // var e = document.getElementById("aws-region");
 // var region = e.options[e.selectedIndex].value;
 var region = jQuery('#aws-region').find("option:selected").text();
    ...
    jQuery.ajax({
        ...
        success: function (response) {
           ...
           if(response != 'failure'){
             swal('Connected!', 'Amazon S3 is successfully Authenticated', 'success');
                     document.getElementById('bucket-button').style.display = 'block';
                     document.getElementById('aws-bucket-div').style.display = 'block';
                     document.getElementById('aws_sync').style.display = 'block';

           }else if(response == 'failure'){
                    document.getElementById('bucket-button').style.display = 'none';
                     document.getElementById('aws-bucket-div').style.display = 'none';
                     document.getElementById('aws_sync').style.display = 'none';
            swal('Warning!', 'Please provide Valid Credentials', 'warning');
           }
        },
        error: function (errorThrown) {
            console.log(errorThrown);
...
function aws_fetch_buckets(){
    ...
    var secretkey = document.getElementById('aws-secret-key').value;
    var region = jQuery('#aws-region').find("option:selected").text();
    jQuery('#floader').addClass('loader');
    ...
    jQuery.ajax({
        ...
        success: function (response) {
            jQuery('#floader').removeClass('loader');

           if(response != 'failure'){
               var obj = JSON.parse(response);
               var options = "<option>--Select--</option>";
                   for(var i=0; i<=obj.length -1; i++){
                           options += '<option>' + obj[i] + '</option>';
                        }
             var opt = '<span> <label class="import-textnew">Choose Bucket</label> </span> <span class="select_box" style="width:200px;height:40px;"><select onchange="aws_bucket()" class="search_dropdown selectpicker" id="aws-bucket" name ="posttype" style="width:200px;height:39px;">'+ options +' </select> </span>';
                               jQuery('#aws-bucket-div').html(opt);
                               var aws_bucket_options = document.getElementById('aws-bucket').innerHTML;
                                    jQuery.ajax({
                                        type : 'POST',
                                        url : ajaxurl,
                                        data: {
                                            'action' : 'aws_bucket_options',
                                            'options' : aws_bucket_options
                                        },
                                        success: function (data){
                                            //alert(data);
                                        },
            ...
            swal('Warning!', 'Please check the permission of the buckets', 'warning')
            }
        },
        error: function (errorThrown) {
            console.log(errorThrown);
...
function aws_bucket(){
    var bucket = jQuery('#aws-bucket').find("option:selected").text();

  jQuery.ajax({
        type: 'POST',
        url: ajaxurl,
        data: {
            'action': 'aws_save_bucketname',
            'name' : bucket
        },
        success: function (response) {
           if(response != 'failure'){
             swal('Success!', 'Your Bucket is successfully added', 'success')
           }
...
function wordss_bac(){
    //var check = jQuery('#wordpress_check').val();
    alert('check');
}
jQuery("#wordss_bac").click(function(){
...
function s3_sync(){
       ...
       if(s3region == '--Select--'){
        ...
        var s3region = jQuery('#aws-bucket').val();
        if(s3region == '--Select--'){
          swal('Warning!', 'Please select a bucket', 'warning');
          return false;
        }
    var slimit = document.getElementById('slimit').value;
    jQuery('#sfloader').addClass('syloader');   
    jQuery.ajax({
        type : 'POST',
        url : ajaxurl,
        data: {
        ...
        success: function (data){
             ...
             //parent.document.getElementById("progressbar").innerHTML ='<div style=\"width:'+data.percent+';background:linear-gradient(to bottom, rgba(125,126,125,1) 0%,rgba(14,14,14,1) 100%); ;height:35px;\">&nbsp;</div>';
            if(data.percent == '100%')      
             parent.document.getElementById("information").innerHTML='<div style=\"text-align:center; font-weight:bold\">Process Completed.</div>';
            else
             parent.document.getElementById("information").innerHTML='<div style=\"text-align:center; font-weight:bold\">'+data.percent+' is processed..</div>';
            document.getElementById('slimit').value = parseInt(data.elimit) + 1;
            if(data.max_limit_reach == 'yes'){
                jQuery('#sfloader').removeClass('syloader');    
                swal('Success!', 'Sync completed.', 'success');
                return false;
            }else{
                s3_sync();
            }
