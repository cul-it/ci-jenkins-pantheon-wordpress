...
class WPToolsetImporter extends ToolsetImporter
{
	...
	function InsertUpdateData($postId,$metaKey,$fieldType,$fieldValue)
	{
		...
		if (!empty($isRepetitive) && $isRepetitive == 1 &&$fieldType!='image') {
			$valuesArray =$this->explodeFunction('|',$fieldValue);
			foreach ($valuesArray as $values) {
				$values=trim($values);
            ...
            foreach($valuesArray as $values) {
                $values=trim($values);
                if ( preg_match_all( '/\b(?:(?:https?|ftp|file):\/\/|www\.|ftp\.)[-A-Z0-9+&@#\/%=~_|$?!:,.]*[A-Z0-9+&@#\/%=~_|$]/i',$values, $matchedlist, PREG_PATTERN_ORDER ) ) {
                      $attachid = $uci_admin->set_featureimage( $values, $postId );
