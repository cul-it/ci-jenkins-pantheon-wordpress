...
class SmackUCIWooCommerceHelper {
	...
	public function importWooCommerceProducts ($data_array, $importType, $mode, $eventKey, $duplicateHandling) {
		...
		/*if(isset($data_array['PRODUCTSKU']) && $data_array['PRODUCTSKU'] != '' && $data_array['PRODUCTSKU'] != null) {SmackUCIWooCommerceHelperSmackUCIWooCommerceHelper
			...
			switch ($ekey) {
				...
				case 'thumbnail_id' :
					if (is_numeric($data_array[$ekey])) {
						...
						$feature = trim($data_array[$ekey]);
						if ( preg_match_all( '/\b(?:(?:https?|ftp|file):\/\/|www\.|ftp\.)[-A-Z0-9+&@#\/%=~_|$?!:,.]*[A-Z0-9+&@#\/%=~_|$]/i',$feature, $matchedlist, PREG_PATTERN_ORDER ) ) {
                            $attachid = $uci_admin->set_featureimage( $feature, $pID );
				...
				case '_shipping_class' :  //kavin added 
					global $wpdb;
					$value = $data_array[$ekey]; 
					$results= $wpdb->get_row("select term_id from ".$wpdb->prefix."terms where name='".$value."'");
					$id=$results->term_id;
					if(!empty($id)) {
						$wpdb->query("update ".$wpdb->prefix."term_taxonomy set count=1 where `term_taxonomy_id`='".$id."'");
						$wpdb->query("insert into ".$wpdb->prefix."term_relationships(object_id,term_taxonomy_id)values('".$pID."','".$id."')"); 
					}
					break;
				case  '_variation_description' :
					$value = $data_array[$ekey];
					update_post_meta($pID,'_variation_description',$value);
					break;
				   ...
				   if($mode!='Update') {
                    ...
                    foreach ($value_order_item_meta[$key] as $itemkey => $itemvalue) {
                        wc_add_order_item_meta($oid, $itemkey, $itemvalue);
                                ...
                                elseif($mode =='Update') {
                                    global $wpdb;
                                    $id = $wpdb->get_row("select order_item_id from ".$wpdb->prefix."woocommerce_order_items where order_id='$pID'");
                                    $ids = $id->order_item_id;
                                    wc_update_order_item($pID, $value);
                                    foreach ($value_order_item_meta[$key] as $itemkey => $itemvalue) {
                                    wc_update_order_item_meta($ids, $itemkey, $itemvalue);
                                    }
                                }
			                }
		                }
