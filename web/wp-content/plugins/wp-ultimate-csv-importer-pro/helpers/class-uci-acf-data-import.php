			...
			foreach ($acf_field as $key => $value) {
				if (array_key_exists($value, $data_array)) {
					...
					if ($acf_type[$value] == 'post_object') {
						$vals=str_replace(' ','',$data_array[$value]);
						$data_value = explode(',', $vals);
                   ...
                   if ($acf_type[$value] == 'select') {
					...
					if($acf_type[$value] == 'link') {
						$vals  = explode(',',$data_array[$value]);
						if($vals[0])  {
							$vals['url']=$vals[0];
							unset($vals[0]);
						}
						...
						$data_array[$value] =$vals;
					}
					if ($acf_type[$value] == 'image') {
						if ($acf_image_import_method[$key] == 'url' || $acf_image_import_method[$key] == 'array') {
							$data_array[$value] = trim($data_array[$value]);
							$ext = pathinfo($data_array[$value], PATHINFO_EXTENSION);
							if($ext== 'jpg' || $ext == 'jpeg' || $ext == 'png') {
								$img_id = $wpdb->get_col($wpdb->prepare("select ID from $wpdb->posts where guid = %s AND post_type='attachment'",$data_array[$value]));
								if(!empty($img_id)) {
									$data_array[$value]=$img_id[0];
								}
								else {
					...
					if ($acf_file_import_method[$key] == 'url' || $acf_file_import_method[$key] == 'array' ) {
							$data_array[$value] = trim($data_array[$value]);
					...
					if ($acf_type[$value] == 'gallery') {#TODO gallery fields
						$gallery_ids =array();
						...
						foreach($exploded_gallery_items as $gallery) {
							...
							if(preg_match_all('/\b(?:(?:https?|ftp|file):\/\/|www\.|ftp\.)[-A-Z0-9+&@#\/%=~_|$?!:,.]*[A-Z0-9+&@#\/%=~_|$]/i', $gallery,$matched_gallerylist,PREG_PATTERN_ORDER)){
								...
								if($ext == 'jpg' || $ext == 'jpeg' || $ext == 'png') {
									$img_id = $wpdb->get_col($wpdb->prepare("select ID from $wpdb->posts where guid = %s AND post_type='attachment'",$gallery));
									if(!empty($img_id)) {
					...
					if (in_array($importAs, $listTaxonomy)) {
						if($term_meta = 'yes'){
							update_term_meta($pID, $value, $data_array[$value]);
              ...
              $acf_rep_field_info['_'.$rep_field_meta_key]=$repeater_fields[$repKey];
						...
						foreach ( $exploded_gallery_items as $gallery => $gval) {
							...
							if ( preg_match_all( '/\b(?:(?:https?|ftp|file):\/\/|www\.|ftp\.)[-A-Z0-9+&@#\/%=~_|$?!:,.]*[A-Z0-9+&@#\/%=~_|$]/i', $gval ) ) {
								$ext = pathinfo($gval, PATHINFO_EXTENSION);
								if($ext == 'jpg' || $ext == 'jpeg' || $ext == 'png') {
								$img_id = $wpdb->get_col($wpdb->prepare("select ID from $wpdb->posts where guid = %s AND post_type='attachment'",$gval));
								if(!empty($img_id)) {
									$gallery_ids[$gallery] = $img_id[0];
								}else    {
									$get_gallery_id = $uci_admin->set_featureimage( $gval, $pID,'');
									if($get_gallery_id != '') {
										$gallery_ids[$gallery] = $get_gallery_id;
									}
					...
					if(preg_match_all('/\b(?:(?:https?|ftp|file):\/\/|www\.|ftp\.)[-A-Z0-9+&@#\/%=~_|$?!:,.]*[A-Z0-9+&@#\/%=~_|$]/i', $image_link)){
						...
						if($ext== 'jpg' || $ext == 'jpeg' || $ext == 'png') {
							$img_id = $wpdb->get_col($wpdb->prepare("select ID from $wpdb->posts where guid = %s AND post_type='attachment'",$image_link));
							if(!empty($img_id)) {
								$acf_rep_field_info[$rep_field_meta_key]=$img_id[0];
							}else {
					...
					if(preg_match_all('/\b(?:(?:https?|ftp|file):\/\/|www\.|ftp\.)[-A-Z0-9+&@#\/%=~_|$?!:,.]*[A-Z0-9+&@#\/%=~_|$]/i', $image_link)){
						$ext = pathinfo($image_link, PATHINFO_EXTENSION);
						if($ext== 'pdf' || $ext == 'mp3' || $ext == $ext) {
							$fil_id = $wpdb->get_col($wpdb->prepare("select ID from $wpdb->posts where guid = %s AND post_type='attachment'",$image_link));
							if(!empty($fil_id)) {
