...
class SmackUCIScheduleManager
{
	...
	//print_r($_POST);die;
        ...
        $storeSchedule = $wpdb->insert('wp_ultimate_csv_importer_scheduled_import',
            ...
            array('%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s','%d', '%s','%d', '%s', '%s', '%s')
        );
        ...
        if(!is_wp_error($wpdb->insert_id)) {
            if ($import_method == 'url') {
                ...
                if ($import_method == 'ftp') {
                    ...
                    $save_ftpfile = "insert into wp_ultimate_csv_importer_ftp_schedules (schedule_id, hostname, username, password, initial_path, filename,hosttype) values ('{$wpdb->insert_id}', '{$post_values[$eventKey]['import_file']['host_name']}', '{$post_values[$eventKey]['import_file']['host_username']}', '{$post_values[$eventKey]['import_file']['host_password']}', '{$post_values[$eventKey]['import_file']['host_path']}', '{$eventKey}','{$post_values[$eventKey]['import_file']['host_type']}')";
    ...
    public static function smack_uci_cron_scheduler() {
        ...
        if (!empty($scheduleList)) {
			...
			$ftp_hosttype = $ftp_scheduleList[0]->hosttype;
                        //$ftp_filename = $ftp_scheduleList[0]->filename;
                        $ftp_filename = basename($ftp_scheduleList[0]->initial_path);
                        $returnData = $ftpObj->ftpfile_handling($ftp_hostname, $ftp_port, $ftp_username, $ftp_password, $ftp_filename, $ftp_hostpath,$ftp_hosttype);
                        if (isset($returnData['Failure'])) {
                            $proceed_scheduling = 0;
                        } else {
                            $data['csv_name'] = $returnData['eventkey'];
