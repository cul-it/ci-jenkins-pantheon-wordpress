...
class SmackUCIFtpHandler {
	...
	public static function init() {
		...
		$host_type = isset($ftp_records['host_type']) ? $ftp_records['host_type'] : 'ftp';
		$result = self::ftpfile_handling($server,$port,$username,$password,$ftp_file_name,$file_path,$host_type);
		echo json_encode($result);die;

	}
	public static function ftpfile_handling($server, $port, $username, $password, $ftp_file_name,$file_path , $host_type) {
		...
		try {
			...
			if($ftp->get($file_path, $local_file , $host_type)) {
...
class SmackUCISFTP {
	 ...
	 * @return bool
	 */
	public function connect($hosttype) {
		// check for ftp
		if($hosttype == 'ftp') {
			if(!$this->_stream = ftp_connect($this->_host, $this->_port, $this->_timeout)) {
				$this->error = "Failed to2 connect to {$this->_host}";
				return false;
			}
		}
		 // check for ftp SSL/TLS 
		if($hosttype == 'ftps') {
			if(function_exists("ftp_ssl_connect")) {
		      	     if(!$this->_stream = ftp_ssl_connect($this->_host)) {
		 ...
		 // check for ssh2/sftp
		if($hosttype == 'ssh2') {			
				if(!$this->_stream = ssh2_connect($this->_host, $this->_port)){
					$this->error = "Failed to connect to {$this->_host}";
					return false;
				}
		}	
		if($hosttype == 'ssh2'){
			if(ssh2_auth_password($this->_stream, $this->_user, $this->_pwd)) {
				$sftp = ssh2_sftp($this->_stream);
				@ftp_pasv($this->_stream, TRUE);
				$this->system_type = ftp_systype($sftp);
				return true;
			} else {
			...
			if(@ftp_login($this->_stream, $this->_user, $this->_pwd)) {
				...
				$this->error = "Failed to2 connect to {$this->_host} (login failed)";
				return false;
			}
		}
	}
	 ...
	 * @return bool
	 */
	public function get($remote_file = null, $local_file = null, $hosttype, $mode = FTP_ASCII ) {
		if($hosttype == 'ssh2'){
			   if($stream = @fopen("ssh2.sftp://".intval($this->_stream).$remote_file, 'r')){
				$local = @fopen($local_file, 'w+');
				$remote = fread($stream, 1024);
				fwrite($local, $remote);
				fclose($stream);
				fclose($local);		
				return true ;
