...
// TODO - add a param and allow the same in if statement for CRON JOB(Schedule Import/Export)
...
if (is_plugin_active('wp-ultimate-csv-importer-pro/wp-ultimate-csv-importer-pro.php')) {
    ...
    if (in_array($_REQUEST['page'], $plugin_pages) || in_array($_REQUEST['action'], $plugin_ajax_hooks)) {
...
class SmackCSV
{
    ...
    public function show_admin_menus()
    {
        ...
        if ($current_user->user_login == 'admin' || get_current_user_id() == 1 || current_user_can('csv_importer_pro')) {
    ...
    public function load_functionalities()
    {
        ...
        $my_page = add_menu_page('Ultimate CSV Importer PRO', 'Ultimate CSV Importer PRO', 'manage_options',
            'com.smackcoders.csvimporternew.menu', array(__CLASS__, 'load_menu'), plugins_url("assets/images/wp-ultimate-csv-importer.png", __FILE__));
    ...
    public function csv_enqueue_function()
    {
        ...
        wp_register_script(SmackCSV::$plugin_instance->getPluginSlug() . 'script_csv_importer', plugins_url('assets/js/admin.js', __FILE__), array('jquery'));
    ...
    {
        global $wpdb;
        $get_result = $wpdb->get_results("SELECT post_id FROM {$wpdb->prefix}ultimate_csv_importer_shortcode_manager", ARRAY_A);
