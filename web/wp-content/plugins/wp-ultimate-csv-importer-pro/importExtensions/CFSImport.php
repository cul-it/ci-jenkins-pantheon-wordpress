...
class CFSImport {
    ...
    function set_cfs_values($header_array ,$value_array , $map, $post_id , $type){
		...
		$this->cfs_import_function($post_values, $post_id , $header_array , $value_array);

    }

    public function cfs_import_function ($data_array, $pID , $header_array , $value_array) {
		...
		foreach ($data_array as $dkey => $dvalue) {
			if(array_key_exists($dkey,$cfs_data['CFS'])){
				...
				elseif($cfs_data['CFS'][$dkey]['type'] == 'file'){
					$darray[$cfs_data['CFS'][$dkey]['name']] = $media_instance->media_handling($dvalue, $pID,$data_array,'','','',$header_array,$value_array);
				...
				elseif($cfs_data['CFS'][$dkey]['type'] == 'term'){
					...
					foreach($relationterms as $rtk => $rtv){
						//$termid = $helpers_instance->get_requested_term_details($pID, $rtv);
