...
class WooCommerceMetaImport {
	...
	public function woocommerce_meta_import_function ($data_array, $pID , $import_type , $line_number , $mode, $header_array , $value_array , $core_array) {
		global $wpdb;
		...
		foreach ($data_array as $ekey => $eval) {
			switch ($ekey) {
				...
				case 'default_attributes' :
					if ($data_array[$ekey]) {
						...
						foreach($dattribute as $dattrkey){
							...
							$def_attri_value = $wpdb->get_var("SELECT slug FROM {$wpdb->prefix}terms WHERE name = '$def_attribute[1]' ");
				...
				case 'custom_attributes' :
					if ($data_array[$ekey]) {
						...
						foreach($cusattribute as $cusattrkey){
							...
							$cus_attri_value = $wpdb->get_var("SELECT slug FROM {$wpdb->prefix}terms WHERE name = '$cus_attribute[1]' ");
				...
				case 'limit_usage_to_x_items' :
					...
					break;
				case 'expiry_date' :
					//$metaData['expiry_date'] = strtotime($data_array[$ekey]);
				...
				for ($att = 0; $att < $attr_count; $att++) {
					...
					if (!empty($product_attributes)) {
						...
						if(!empty($cusAttribute)){
							...
							/////
							// $update_detail = $wpdb->get_col($wpdb->prepare("select post_title from $wpdb->posts where ID = %d", $productID));
							// $post_title = $update_detail[0];
							// $update_post_array=array();
							// $update_post_array['ID'] = $pID;
							// $update_post_array['post_title'] = $post_title . '-' . $cusAttval;
		...
		if (isset($metaData['_product_attributes']) && !empty($metaData['_product_attributes']) && $import_type !== 'WooCommerce Product Variations') {
			...
			foreach($metaData['_product_attributes'] as $attrKey => $attrVal) {			
				...
				elseif($get_csvpro_settings['woocomattr'] == 'false'){
					...
					if(empty($get_attributeLabel)){
						//unset($metaData['_product_attributes'][$attrKey]);
					}else {
						$attrId = $get_attributeLabel[0]->attribute_id;
						$custom_attribute_name = 'pa_' . $attrVal['name'];	
                    	$split_line = explode('|', $attr_data);
