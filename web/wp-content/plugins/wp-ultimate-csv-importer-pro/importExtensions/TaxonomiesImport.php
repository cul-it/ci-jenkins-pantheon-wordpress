    ...
    public function taxonomies_import_function ($data_array, $mode, $importType , $check , $hash_key , $line_number ,$header_array ,$value_array) {
		...
		if($mode == 'Insert'){
			if(!empty($termID)){
				...
				return array('MODE' => $mode, 'ERROR_MSG' => 'The term already exists!');
					...
					if(is_wp_error($taxoID)){
						...
						if(isset($_image)){
							...
							elseif($importType == 'event-categories' || $importType == 'event-tags'){
								...
								if($importType == 'event-categories'){
									$fields = $wpdb->insert( $events_table , array('object_id' =>  $termID, 'meta_key' => 'category-image', 'meta_value' => $guid, 'meta_date' => $date) );
									$fields = $wpdb->insert( $events_table , array('object_id' =>  $termID, 'meta_key' => 'category-image-id', 'meta_value' => $imageid, 'meta_date' => $date) );
								}elseif($importType == 'event-tags'){
									$fields = $wpdb->insert( $events_table , array('object_id' =>  $termID, 'meta_key' => 'tag-image', 'meta_value' => $guid, 'meta_date' => $date) );
									$fields = $wpdb->insert( $events_table , array('object_id' =>  $termID, 'meta_key' => 'tag-image-id', 'meta_value' => $imageid, 'meta_date' => $date) );
								}
							}
						}
						...
						if(isset($_color)){
							if($importType == 'event-categories'){
								$fields = $wpdb->insert( $events_table , array('object_id' =>  $termID, 'meta_key' => 'category-bgcolor', 'meta_value' => $_color, 'meta_date' => $date) );
							}elseif($importType == 'event-tags'){
								$fields = $wpdb->insert( $events_table , array('object_id' =>  $termID, 'meta_key' => 'tag-bgcolor', 'meta_value' => $_color, 'meta_date' => $date) );
							}
						}
			...
			if($mode == 'Update') {
				if(!empty($termID)){
					...
					if($importType == 'event-categories' || $importType == 'event-tags'){
						$fields =  $wpdb->get_results("UPDATE {$wpdb->prefix}terms SET `name` = '$_name' , `slug` = '$_slug' WHERE `term_id` = '$termID'");
						$fields =  $wpdb->get_results("UPDATE $terms_table SET `description` = '$_desc' WHERE `term_id` = '$termID'");
					}
					else{
					...
					if(isset($_image)){
							...
							if($importType == 'product_cat'){
								...
								if($importType == 'event-categories'){
									$fields =  $wpdb->get_results("UPDATE $events_table SET `meta_value` = '$guid' , `meta_date` = '$date' WHERE `object_id` = '$termID' and `meta_key` = 'category-image'");
									$fields =  $wpdb->get_results("UPDATE $events_table SET `meta_value` = $_img  , `meta_date` = '$date' WHERE `object_id` = '$termID' and `meta_key` = 'category-image-id'");
									
								}elseif($importType == 'event-tags'){
									$fields =  $wpdb->get_results("UPDATE $events_table SET `meta_value` = '$guid' , `meta_date` = '$date' WHERE `object_id` = '$termID' and `meta_key` = 'tag-image' ");
									$fields =  $wpdb->get_results("UPDATE $events_table SET `meta_value` = $_img  , `meta_date` = '$date' WHERE `object_id` = '$termID' and `meta_key` = 'tag-image-id' ");
								}
							}				
					}
            ...
            update_term_meta($termID , 'display_type' , $_display_type);
					...
					if(isset($_color)){
						if($importType == 'event-categories'){
							$fields =  $wpdb->get_results("UPDATE $events_table SET `meta_value` = '$_color' , `meta_date` = '$date' WHERE `object_id` = '$termID' and `meta_key` = 'category-bgcolor' ");
						}elseif($importType == 'event-tags'){
							$fields =  $wpdb->get_results("UPDATE $events_table SET `meta_value` = '$_color' , `meta_date` = '$date' WHERE `object_id` = '$termID' and `meta_key` = 'tag-bgcolor' ");
						}
					}
