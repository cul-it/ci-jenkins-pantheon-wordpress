...
class TermsandTaxonomiesImport {
	...
	function set_terms_taxo_values($header_array ,$value_array , $map , $post_id , $type, $mode , $line_number = null, $lang_map = null){
		$post_values = [];
		$helpers_instance = ImportHelpers::getInstance();
		$post_values = $helpers_instance->get_header_values($map , $header_array , $value_array);
		$lang_values = $helpers_instance->get_header_values($lang_map , $header_array , $value_array);
		$this->terms_taxo_import_function($post_values,$type, $post_id , $mode , $line_number,$lang_values);

	}

	public function terms_taxo_import_function ($data_array, $type ,$pID , $mode , $line_number,$lang_values) {
		...
		foreach ($data_array as $termKey => $termVal) {
			...
			if (strpos($termKey, 'pa_') !== false) {
				if($termKey == 'product_category' || $termKey == 'product_tag'){
...
#TODO
			...
			switch ($termKey) {
				case 'post_category' :
					...
					if(isset($categories[$termKey]) && $categories[$termKey] != '')
						$this->assignTermsAndTaxonomies($categories, $category_name, $pID,$lang_values);
				...
				case 'post_tag' :
					...
					if($mode == 'Update'){
						...
						$this->assignTermsAndTaxonomies($tags, $tag_name, $pID,$lang_values);
					    if(preg_match("(Can't|Skipped|Duplicate)", $core_instance->detailed_log[$line_number]['Message']) === 0){
				...
				default :
					...
					if(isset($smack_taxonomy[$termKey]) && $smack_taxonomy[$termKey] != '')
						$this->assignTermsAndTaxonomies($smack_taxonomy, $taxonomy_name, $pID,$lang_values);
					break;
			}
		}
	...
	public function assignTermsAndTaxonomies($categories, $category_name, $pID, $data_array ) {
