...
class SaveMapping{
			...
			if (($h = fopen($upload_dir.$hash_key.'/'.$hash_key, "r")) !== FALSE) 
			{
				...
				while(($data = fgetcsv($h, 0, $delimiters[$array_index]))!== FALSE) {
					...
					if ($i >= $line_number && $i <= $limit) {
						$value_array = $info[$i];
						$get_arr = $this->main_import_process($map , $header_array ,$value_array , $selected_type , $get_mode, $i , $check , $hash_key);
						$post_id = $get_arr['id'];	
				...
				$running = $wpdb->get_row("SELECT running FROM $log_table_name WHERE hash_key = '$hash_key' ",ARRAY_A);
	...
	// 	$template_table_name = $wpdb->prefix ."ultimate_csv_importer_mappingtemplate";
	// 	$log_table_name = $wpdb->prefix ."import_detail_log";
	// 	update_option('sm_bulk_import_page_number', $pageNumber);
	// 	update_option('sm_bulk_import_line_number', $line_number);
	// 	$response = [];	
	// 	$background_values = $wpdb->get_results("SELECT mapping , module  FROM $template_table_name WHERE `eventKey` = '$hash_key' ");	
	// 	foreach($background_values as $values){
	// 		$mapped_fields_values = $values->mapping;	
	...
	// 		if($running == 0){
	// 			$response['success'] = false;
	// 			$response['message'] = 'Record Paused';
	// 			echo wp_json_encode($response);
	// 			wp_die();
	// 		}
	// 		$data_array = str_replace( "\\", "", $data_array );
	// 		$json = json_decode($data_array,true);
	// 		$data_array = json_decode(json_encode($json),true);
	// 		$get_arr = $this->main_import_process($map , $header_array ,$data_array['data'] , $selected_type , $get_mode, $line_number , $check , $hash_key);
	// 		$post_id = $get_arr['id'];	
	// 		$core_instance->detailed_log = $get_arr['detail_log'];
	// 		$helpers_instance->get_post_ids($post_id ,$hash_key);
	// 		$remaining_records = $total_rows - $line_number;
	// 		$wpdb->get_results("UPDATE $log_table_name SET processing_records = 1 , remaining_records = $remaining_records , status = 'Processing' WHERE hash_key = '$hash_key'");			
	// 		if($line_number == $total_rows){
