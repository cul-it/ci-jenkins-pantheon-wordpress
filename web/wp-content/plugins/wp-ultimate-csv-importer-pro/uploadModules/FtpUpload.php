        ...
        if(!$conn_id){
            ...
            if($validate_format == 'yes'){
                ...
                if($upload_dir){
                    ...
                    $fields = $wpdb->insert( $file_table_name , array( 'file_name' => $ftp_file_name , 'hash_key' => $event_key , 'status' => 'Downloading' , 'lock' => true  ) );
                    ...
                    if($dld > 0){
                        $i = ($dld/$fs)*100;
                        $fields = $wpdb->get_results("UPDATE $file_table_name SET  progress='$i' , `lock`=true WHERE id = '$lastid'");
                    }
                    ...
                    if($ret == FTP_FINISHED){
                        ...
                        if($validate_file == "yes"){
                            ...
                            echo wp_json_encode($response); 
                            unlink($path);
                            $fields = $wpdb->get_results("UPDATE $file_table_name SET status='Download Failed' WHERE id = '$lastid'");
                        }
                    } else {
                        $fields = $wpdb->get_results("UPDATE $file_table_name SET status='Download Failed' WHERE id = '$lastid'");
            ...
            if($validate_format == 'yes'){
                ...
                if($ret == FTP_FINISHED){
                    ...
                    if($validate_file == "yes"){
                        ...
                        $fields = $wpdb->get_results("UPDATE $file_table_name SET status='Downloaded',`lock`=false WHERE id = '$lastid'");
                        $get_result = FtpUpload::$smack_csv_instance->import_record_function($event_key , $ftp_file_name);
                        $response['success'] = true;
                        $response['filename'] = $ftp_file_name;
                        $response['hashkey'] = $event_key;
                        $response['posttype'] = $get_result['Post Type'];
