                ...
                if($upload_dir){
                    ...
                    if(ssh2_auth_password($connection, $host_username, $host_password)){
                        ...
                        if($sftp){
                            ...
                            $fields = $wpdb->insert( $file_table_name , array( 'file_name' => $name , 'hash_key' => $event_key , 'status' => 'Downloading' , 'lock' => true ) );
                ...
                if($upload_dir){
                    ...
                    if(ssh2_auth_password($connection, $host_username, $host_password)){
                        ...
                        if($sftp){
                            ...
                            $fields = $wpdb->insert( $file_table_name , array( 'file_name' => $name , 'hash_key' => $event_key , 'status' => 'Downloading' , 'lock' => true, 'mode' => $mode_of_action ) );
                            $last_id = $wpdb->get_results("SELECT id FROM $file_table_name ORDER BY id DESC LIMIT 1",ARRAY_A);
                            $lastid=$last_id[0]['id'];
                            $local_file = $upload_dir.$event_key.'/'.$event_key;
                            //chmod($local_file, 0777);
