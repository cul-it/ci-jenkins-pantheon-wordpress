...
class UrlUpload implements Uploads{
    ...
    public function externalfile_handling($url){
		...
		// 	$response['message'] = 'Not a valid url';
		...
		// }else{
			...
			if($pub=='pubhtml'){
				...
				//throw new Exception('Update your Google sheet as Public');
				//echo 'Update your Google sheet as Public'; 
				$response['success'] = false;
			...
			if($validate_format == 'yes'){
				...
				if($upload_dir){
					...
					$fields = $wpdb->insert( $file_table_name , array( 'file_name' => $url_file_name , 'hash_key' => $event_key , 'status' => 'Downloading','lock' => true, 'mode' => $mode_of_action ) );
