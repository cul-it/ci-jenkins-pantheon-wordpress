...
class FileManager {
    ...
    public function downloadAllFiles(){
		...
		for($i=0;$i<$count;$i++) {
           ...
           // $filePath= $pathList[$i] . '/' . $eventkeys[$i] . '.' . $file_extension;
    ...
    public function deleteFiles()
	{
		...
		//$filekey = $wpdb->get_col($wpdb->prepare("select eventKey,id from smackuci_events where revision=%d and original_file_name=%s", $version,$filename));
    ...
    public function delete_all_files() {
		...
		for($i=0;$i<$count;$i++){
			...
			if(file_exists($details[$i])) {
				...
				//$file_txt_path[] = $upload_path . $get_eventkey[$i] .'.txt'; 
			}
		}
		...
		//$file = file_get_contents($upload_path . $get_eventkey .'.'. $file_extension, true);
		...
		if(!empty($records[0]) && !empty($module)):
			...
			//if(is_array($file_txt_path)) {	
				$count = count($file_path);
				//$count = count($file_txt_path);
			}
			if(!empty($file_path)) {
			//if(!empty($file_txt_path)) {	
				for($i=0;$i<$count;$i++) {		
					if(file_exists($file_path[$i])) {
					//if(file_exists($file_txt_path[$i])) {

						$file_path = $file_path[$i];
						//$file_txt_path = $file_txt_path[$i];
						
						//array_map('unlink', glob("$file_path"));
						//array_map('unlink' , glob("$file_txt_path"));
						array_map('unlink', glob("$file_path/*.*"));
						array_map('unlink', glob("$file_path/**"));
						rmdir($file_path);
									//unlink($file_path);				
	...
	/**
	 * Returns inserted,updated,skipped count.
